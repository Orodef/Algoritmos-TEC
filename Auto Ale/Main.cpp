...
void iniciarBonus() {
	for (int i = 0; i < numeroBonus; i++) {
		...
		//Segmento de sensores para el PILOTO AUTOMÁTICO
...
bool sensorPosicion() {
	for (int i = 0; i < numeroBalasEnemigo; i++) {
		...
		if (balasEnemigo[i] != NULL) {
			if (personaje->x == balasEnemigo[i]->x) return true;
			else return false;
		}
	}
}
...
void moverAutomatico() {
	if (sensorPosicion) {
		...
		timer = al_create_timer(1.0 / FPS7);
...
int main(int argc, char **argv) {
	...
	fuente = al_load_font("JacobiStriped.ttf", 30, NULL);
